/*
 * drivers/usb/host/ehci-wiiu.c
 *
 * Nintendo Wii U USB Enhanced Host Controller Interface.
...
#define DRV_MODULE_NAME "ehci-wiiu"
#define DRV_DESCRIPTION "Nintendo Wii EHCI Host Controller"
#define DRV_AUTHOR      "Albert Herranz"

#define WIIU_EHCI_CTL		0x0d0400cc
#define WIIU_EHCI_CTL_INTE	(1<<15)


/* called during probe() after chip reset completes */
static int ehci_wiiu_reset(struct usb_hcd *hcd)
{
	...
	*/
	...
	out_be32(ehci_ctl, in_be32(ehci_ctl) | WIIU_EHCI_CTL_INTE);
	iounmap(ehci_ctl);
...
static const struct hc_driver ehci_wiiu_hc_driver = {
	.description		= hcd_name,
	.product_desc		= "Nintendo Wii U EHCI Host Controller",
...
static int ehci_hcd_wiiu_probe(struct platform_device *op)
{
	...
	hcd = usb_create_hcd(&ehci_wiiu_hc_driver, &op->dev, DRV_MODULE_NAME);
	if (!hcd) {
		error = -ENOMEM;
		goto out;
	}
...
static int ehci_hcd_wiiu_remove(struct platform_device *op)
{
...
static void ehci_hcd_wiiu_shutdown(struct platform_device *op)
{
	struct usb_hcd *hcd = dev_get_drvdata(&op->dev);
...
static struct of_device_id ehci_hcd_wiiu_match[] = {
	{
		.compatible = "nintendo,wiiu-usb-ehci",
	},
	{},
};
MODULE_DEVICE_TABLE(of, ehci_hcd_wiiu_match);

static struct platform_driver ehci_hcd_wiiu_driver = {
	.driver = {
		.name = DRV_MODULE_NAME,
		.owner = THIS_MODULE,
		.of_match_table = ehci_hcd_wiiu_match,
	},
	.probe		= ehci_hcd_wiiu_probe,
	.remove		= ehci_hcd_wiiu_remove,
	.shutdown	= ehci_hcd_wiiu_shutdown,
};
