...
#define DRV_DESCRIPTION "Nintendo Wii U OHCI Host Controller"
#define DRV_AUTHOR      "Albert Herranz"

#define WIIU_EHCI_CTL 0x0d0400cc	/* vendor control register */
#define WIIU_EHCI_CTL_OH0INTE	(1<<11)	/* oh0 interrupt enable */
#define WIIU_EHCI_CTL_OH1INTE	(1<<12)	/* oh1 interrupt enable */

static int bulk_sleep = 0;
module_param(bulk_sleep, int, 0644);
MODULE_PARM_DESC(bulk_sleep, "Quirk sleep on bulk packets. -1 to disable.");
...
void ohci_wiiu_control_quirk(struct ohci_hcd *ohci)
{
...
void ohci_wiiu_bulk_quirk(struct ohci_hcd *ohci)
{
	/*
    ...
    if (bulk_sleep > 0) {
        udelay(bulk_sleep);
    } else if (bulk_sleep == 0) {
        udelay(250);
    } //else if bulk_sleep < 0; do nothing
}

static int ohci_wiiu_start(struct usb_hcd *hcd)
{
	struct ohci_hcd	*ohci = hcd_to_ohci(hcd);
	void __iomem *ehci_ctl;
	int error;

	ehci_ctl = ioremap(WIIU_EHCI_CTL, 4);
	if (!ehci_ctl) {
	...
	out_be32(ehci_ctl, in_be32(ehci_ctl) |
		 0xe0000 | WIIU_EHCI_CTL_OH0INTE | WIIU_EHCI_CTL_OH1INTE);
	iounmap(ehci_ctl);
...
static const struct hc_driver ohci_wiiu_hc_driver = {
	.description =		hcd_name,
	.product_desc =		"Nintendo Wii U OHCI Host Controller",
...
static int ohci_hcd_wiiu_probe(struct platform_device *op)
{
	...
	hcd = usb_create_hcd(&ohci_wiiu_hc_driver, &op->dev, DRV_MODULE_NAME);
	if (!hcd) {
		error = -ENOMEM;
		goto out;
	}
...
static int ohci_hcd_wiiu_remove(struct platform_device *op)
{
...
static void ohci_hcd_wiiu_shutdown(struct platform_device *op)
{
	struct usb_hcd *hcd = dev_get_drvdata(&op->dev);
...
static struct of_device_id ohci_hcd_wiiu_match[] = {
	{
		.compatible = "nintendo,wiiu-usb-ohci",
	},
	{},
};
MODULE_DEVICE_TABLE(of, ohci_hcd_wiiu_match);

static struct platform_driver ohci_hcd_wiiu_driver = {
	.driver = {
		.name = DRV_MODULE_NAME,
		.owner = THIS_MODULE,
		.of_match_table = ohci_hcd_wiiu_match,
	},
	.probe		= ohci_hcd_wiiu_probe,
	.remove		= ohci_hcd_wiiu_remove,
	.shutdown 	= ohci_hcd_wiiu_shutdown,
};
